...
class HomeController extends Controller
{
    ...
    {
        $d = [
            'totalStudents' => 0, // Default value to prevent undefined variable error
            'totalParents' => 0   // Default value to prevent undefined variable error
        ];

        if (Qs::isAdministratorOrTeacher()) {
            $d['user_type'] = Auth::user()->roles()->first()->name;
            ...
            // For any other user types, provide empty users collection
            $d['users'] = collect([]);
        }
