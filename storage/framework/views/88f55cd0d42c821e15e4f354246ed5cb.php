<div x-data="{ 
    ...
    initializeEvents() {
        ...
        Livewire.on('triggerDownload', () => {
            this.downloadUrl = <?php echo json_encode(session('download_url'), 15, 512) ?>;
            if (this.downloadUrl) {
        ...
        Livewire.on('openPrintWindow', () => {
            this.printUrl = <?php echo json_encode(session('print_url'), 15, 512) ?>;
            if (this.printUrl) {
        ...
        Livewire.on('show-toast', () => {
            this.toastMessage = <?php echo json_encode(session('toast_message'), 15, 512) ?>;
            this.toastType = <?php echo json_encode(session('toast_type'), 15, 512) ?> || 'info';
    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
                ...
                <div class="hidden sm:block">
                    <nav class="flex space-x-4" aria-label="Tabs">
                        <button type="button" 
                            ...
                            class="<?php echo e($selectedTab === 'selection' ? 'bg-green-100 text-green-700' : 'text-gray-500 hover:text-gray-700'); ?> px-3 py-2 font-medium text-sm rounded-md">
                            Select Timetables
                        </button>
                        <button type="button" 
                            wire:click="$set('selectedTab', 'preview')" 
                            class="<?php echo e($selectedTab === 'preview' ? 'bg-green-100 text-green-700' : 'text-gray-500 hover:text-gray-700'); ?> px-3 py-2 font-medium text-sm rounded-md"
                            <?php echo e(!$isConsolidated ? 'disabled' : ''); ?>>
                            Preview Results
                        </button>
                    </nav>
                </div>
        ...
        <div class="border-t border-gray-200">
            <!-- Tab Content -->
            <!--[if BLOCK]><![endif]--><?php if($selectedTab === 'selection'): ?>
                <?php echo $__env->make('livewire.timetable.partials.consolidator-selection', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
            <?php elseif($selectedTab === 'preview' && $isConsolidated): ?>
                <?php echo $__env->make('livewire.timetable.partials.consolidator-preview', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
            <?php elseif($selectedTab === 'preview' && !$isConsolidated): ?>
    ...
    <!-- Error Log Section -->
    <!--[if BLOCK]><![endif]--><?php if(!empty($consolidationErrors)): ?>
        <div class="mt-6">
            <div class="bg-white shadow overflow-hidden sm:rounded-lg">
                <div class="px-4 py-5 sm:px-6 flex justify-between items-center">
                    <div>
                ...
                <div class="border-t border-gray-200">
                    <div class="bg-gray-50 overflow-y-auto max-h-40">
                        <ul class="divide-y divide-gray-200">
                            <!--[if BLOCK]><![endif]--><?php $__currentLoopData = $consolidationErrors; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $error): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <li class="px-4 py-2 text-sm">
                                    <span class="text-gray-500"><?php echo e($error['time']); ?>: </span>
                                    <span class="text-red-600"><?php echo e($error['message']); ?></span>
                                </li>
...
</div> <?php /**PATH C:\projects\MbukuErp\resources\views/livewire/timetable/timetable-consolidator.blade.php ENDPATH**/ ?>