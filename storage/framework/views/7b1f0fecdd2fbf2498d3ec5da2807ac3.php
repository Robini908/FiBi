...
<div class="bg-white dark:bg-gray-50 rounded-lg shadow-sm overflow-hidden" 
     x-data="{ 
        isFormValid: false,
        session: <?php if ((object) ('session') instanceof \Livewire\WireDirective) : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('session'->value()); ?>')<?php echo e('session'->hasModifier('live') ? '.live' : ''); ?><?php else : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('session'); ?>')<?php endif; ?>.defer,
        classId: <?php if ((object) ('classId') instanceof \Livewire\WireDirective) : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('classId'->value()); ?>')<?php echo e('classId'->hasModifier('live') ? '.live' : ''); ?><?php else : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('classId'); ?>')<?php endif; ?>.defer,
        selectedStudents: <?php if ((object) ('selectedStudents') instanceof \Livewire\WireDirective) : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('selectedStudents'->value()); ?>')<?php echo e('selectedStudents'->hasModifier('live') ? '.live' : ''); ?><?php else : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('selectedStudents'); ?>')<?php endif; ?>.defer,
        selectAll: false,
        validate() {
    ...
    <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-200 bg-gray-50 dark:bg-gray-100 flex justify-between items-center">
        <h2 class="font-medium text-xl text-gray-800 dark:text-gray-800 flex items-center">
            ...
            Add Students to <?php echo e($dormName); ?>
...
$message = $__bag->first($__errorArgs[0]); ?> border-red-300 text-red-900 dark:text-red-900 placeholder-red-300 focus:ring-red-500 focus:border-red-500 
                            <?php else: ?> border-gray-300 dark:border-gray-300 focus:ring-emerald-500 focus:border-emerald-500 <?php unset($message);
...
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?>
                    <p class="mt-2 text-sm text-red-600 dark:text-red-600"><?php echo e($message); ?></p>
                <?php unset($message);
if (isset($__messageOriginal)) { $message = $__messageOriginal; }
endif;
unset($__errorArgs, $__bag); ?><!--[if ENDBLOCK]><![endif]-->
...
unset($__errorArgs, $__bag); ?>" 
                    ...
                    <!--[if BLOCK]><![endif]--><?php $__errorArgs = ['classId'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?>
...
unset($__errorArgs, $__bag); ?><!--[if ENDBLOCK]><![endif]-->
        ...
        <div class="mt-6">
            ...
            <!--[if BLOCK]><![endif]--><?php if(session('error')): ?>
                <div class="rounded-md bg-red-50 dark:bg-red-100 p-4 mb-4">
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            <h3 class="text-sm font-medium text-red-800 dark:text-red-900"><?php echo e(session('error')); ?></h3>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-white dark:bg-white shadow rounded-md overflow-hidden">
                <!--[if BLOCK]><![endif]--><?php if(isset($availableStudents) && count($availableStudents) > 0): ?>
                    <ul class="divide-y divide-gray-200 dark:divide-gray-200 max-h-80 overflow-y-auto">
                        <!--[if BLOCK]><![endif]--><?php $__currentLoopData = $availableStudents; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $student): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <li class="px-6 py-4 hover:bg-gray-50 dark:hover:bg-gray-50 transition-colors duration-150">
                                <div class="flex items-center">
                                    ...
                                    >
                                    <label for="student-<?php echo e($student->id); ?>" class="ml-3 block">
                                        <div class="flex items-center">
                                            <div class="flex-shrink-0 h-10 w-10">
                                                <!--[if BLOCK]><![endif]--><?php if($student->photo_by): ?>
                                                    <img class="h-10 w-10 rounded-full object-cover" src="<?php echo e(asset('storage/uploads/'.$student->photo)); ?>" alt="<?php echo e($student->name); ?>">
                                                <?php else: ?>
                                                    <div class="h-10 w-10 rounded-full bg-green-100 dark:bg-green-100 flex items-center justify-center">
                                                        <span class="text-green-600 dark:text-green-600 font-medium text-sm"><?php echo e(substr($student->name, 0, 2)); ?></span>
                                                    </div>
                                                <?php endif; ?><!--[if ENDBLOCK]><![endif]-->
                                            </div>
                                            <div class="ml-4">
                                                <div class="text-sm font-medium text-gray-900 dark:text-gray-800"><?php echo e($student->name); ?></div>
                                                <div class="text-sm text-gray-500 dark:text-gray-600">Adm: <?php echo e($student->adm_no); ?> | Class: <?php echo e($student->my_class->name ?? 'N/A'); ?></div>
                                            </div>
                                        </div>
                                    </label>
                                </div>
                    ...
                    <!-- Pagination -->
                    <!--[if BLOCK]><![endif]--><?php if($availableStudents->hasPages()): ?>
        ...
        <!--[if BLOCK]><![endif]--><?php if(isset($selectedStudents) && is_array($selectedStudents) && count($selectedStudents) > 0): ?>
            <div class="flex items-center bg-green-50 dark:bg-green-50 p-4 rounded-md">
                ...
                <div class="ml-3">
                    <p class="text-sm font-medium text-green-800 dark:text-green-800">
                        <?php echo e(count($selectedStudents)); ?> students selected
                    </p>
                </div>
                <div class="ml-auto">
                    <button 
...
<?php /**PATH C:\projects\MbukuErp\resources\views/livewire/partials/dorms/add-students.blade.php ENDPATH**/ ?>