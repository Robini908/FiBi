<div>
    ...
    <div class="bg-white shadow-sm rounded-lg overflow-hidden mb-6">
        ...
        @include('livewire.attendance.partials.analytics-header')
        ...
        <div class="px-4 py-2 sm:px-6 border-b border-gray-200 bg-gray-50">
            <div class="flex overflow-x-auto space-x-4 pb-1">
                <button wire:click="setTab('overview')" wire:loading.class="opacity-50" wire:loading.attr="disabled" wire:target="setTab" class="px-3 py-2 font-medium text-sm rounded-md whitespace-nowrap {{ $activeTab === 'overview' ? 'bg-green-100 text-green-800' : 'text-gray-500 hover:text-gray-700' }} focus:outline-none">
                ...
                <button wire:click="setTab('trends')" wire:loading.class="opacity-50" wire:loading.attr="disabled" wire:target="setTab" class="px-3 py-2 font-medium text-sm rounded-md whitespace-nowrap {{ $activeTab === 'trends' ? 'bg-green-100 text-green-800' : 'text-gray-500 hover:text-gray-700' }} focus:outline-none">
                ...
                <button wire:click="setTab('students')" wire:loading.class="opacity-50" wire:loading.attr="disabled" wire:target="setTab" class="px-3 py-2 font-medium text-sm rounded-md whitespace-nowrap {{ $activeTab === 'students' ? 'bg-green-100 text-green-800' : 'text-gray-500 hover:text-gray-700' }} focus:outline-none">
                ...
                <button wire:click="setTab('comparison')" wire:loading.class="opacity-50" wire:loading.attr="disabled" wire:target="setTab" class="px-3 py-2 font-medium text-sm rounded-md whitespace-nowrap {{ $activeTab === 'comparison' ? 'bg-green-100 text-green-800' : 'text-gray-500 hover:text-gray-700' }} focus:outline-none">
        ...
        <div class="p-4 sm:p-6">
            <div x-show="true" x-data="{ tab: @entangle('activeTab') }">
                ...
                <div x-show="tab === 'overview'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 transform scale-95" x-transition:enter-end="opacity-100 transform scale-100">
                    ...
                    @include('livewire.attendance.partials.analytics-overview')
                    </div>
                </div>
                ...
                <div x-show="tab === 'trends'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 transform scale-95" x-transition:enter-end="opacity-100 transform scale-100">
                    ...
                    @include('livewire.attendance.partials.analytics-trends')
                    </div>
                </div>
                ...
                <div x-show="tab === 'students'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 transform scale-95" x-transition:enter-end="opacity-100 transform scale-100">
                    ...
                    @include('livewire.attendance.partials.analytics-students')
                    </div>
                </div>
                ...
                <div x-show="tab === 'comparison'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 transform scale-95" x-transition:enter-end="opacity-100 transform scale-100">
                    ...
                    @include('livewire.attendance.partials.analytics-comparison')
                    </div>
                </div>
            </div>
        </div>
...
<script>
    document.addEventListener('livewire:init', function () {
        ...
        if (@this.get('pollingEnabled')) {
            startPolling(@this.get('pollingInterval'));
        }
        
        // Handle polling toggle
        @this.on('polling-toggled', param => {
            if (param.enabled) {
        ...
        @this.on('polling-interval-changed', param => {
            if (@this.get('pollingEnabled')) {
        ...
        document.addEventListener('visibilitychange', () => {
            if (document.visibilityState === 'visible' && @this.get('pollingEnabled')) {
                startPolling(@this.get('pollingInterval'));
            } else {
                stopPolling();
            }
