<div>
                    ...
                    <div class="h-80">
                        @if(!empty($comparisonData))
                            <livewire:livewire-column-chart 
                                key="{{ $classId }}-comparison-{{ now() }}"
                                :column-chart-model="$this->getSectionComparisonChartModel()" 
                            />
                        @else
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @foreach($comparisonData as $data)
                                        <tr class="{{ $data['section_id'] == $sectionId ? 'bg-green-50' : '' }}">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="flex items-center">
                                                    <div class="w-full bg-gray-200 rounded-full h-2.5 mr-2">
                                                        <div class="bg-green-600 h-2.5 rounded-full" style="width: {{ $data['stats']['attendance_rate'] }}%"></div>
                                                    </div>
                                                    <span class="text-sm font-medium text-gray-900">{{ $data['stats']['attendance_rate'] }}%</span>
                                                </div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">
                                                    {{ $data['stats']['present_count'] }}
                                                </span>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800">
                                                    {{ $data['stats']['absent_count'] }}
                                                </span>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-yellow-100 text-yellow-800">
                                                    {{ $data['stats']['late_count'] }}
                                                </span>
                                            </td>
                                        </tr>
                                    @endforeach
    ...
    <div class="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
        ...
        <div class="p-4">
            @if(!$classId || count($comparisonData ?? []) === 0)
            ...
            @else
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div class="bg-green-50 rounded-lg p-4 border border-green-100">
                        ...
                        <p class="text-lg font-bold text-green-700">{{ $bestSection['section_name'] }}</p>
                        <p class="text-sm text-green-600 mt-1">{{ $bestSection['stats']['attendance_rate'] ?? 0 }}% Attendance Rate</p>
                    ...
                    <div class="bg-blue-50 rounded-lg p-4 border border-blue-100">
                        ...
                        <p class="text-lg font-bold text-blue-700">{{ number_format($avgRate, 1) }}%</p>
                    ...
                    <div class="bg-red-50 rounded-lg p-4 border border-red-100">
                        ...
                        <p class="text-lg font-bold text-red-700">{{ $worstSection['section_name'] }}</p>
                        <p class="text-sm text-red-600 mt-1">{{ $worstSection['stats']['attendance_rate'] ?? 0 }}% Attendance Rate</p>
