        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($paginatedDetails as $detail)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-gray-900">
                                    {{ \Carbon\Carbon::parse($detail->attendanceRecord->attendance_date)->format('D, M d, Y') }}
                                </div>
                                <div class="text-sm text-gray-500">
                                    {{ \Carbon\Carbon::parse($detail->attendanceRecord->attendance_date)->format('l') }}
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                                    ...
                                    @elseif($detail->status === 'excused')
                                        bg-blue-100 text-blue-800
                                    @elseif($detail->status === 'sick')
                                        bg-purple-100 text-purple-800
                                    @elseif($detail->status === 'on_leave')
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                                    {{ $detail->attendanceRecord->session_type === 'morning' ? 'bg-blue-100 text-blue-800' : 
                                    ($detail->attendanceRecord->session_type === 'afternoon' ? 'bg-orange-100 text-orange-800' : 'bg-green-100 text-green-800') }}">
                                    {{ ucfirst(str_replace('_', ' ', $detail->attendanceRecord->session_type)) }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
        ...
        @if($totalRecords > 0)
            <div class="px-4 py-3 bg-gray-50 border-t border-gray-200 sm:px-6">
                <div class="flex items-center justify-between">
                    <div class="text-sm text-gray-500">
                        Showing {{ count($paginatedDetails) }} of {{ $totalRecords }} records
                    </div>
