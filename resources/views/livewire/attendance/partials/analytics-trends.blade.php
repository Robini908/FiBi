<div>
    ...
    <div class="bg-white p-4 rounded-lg shadow mb-6">
        <h3 class="text-lg font-medium text-gray-900 mb-4">
            Attendance Trend: {{ ucfirst($trendType) }} {{ $trendType !== 'all' ? 'Rate' : 'Rates' }} ({{ ucfirst($periodType) }})
        </h3>
        
        <div wire:loading.class="opacity-25" class="h-80">
            @if(count($timelineData) > 0)
                <livewire:livewire-column-chart 
                    key="{{ $classId.$sectionId.$trendType.$periodType.now() }}"
                    :column-chart-model="$this->getTrendChartModel()" 
                />
            @else
                ...
                <tr>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">
                            {{ $item['present_count'] }} ({{ $item['total'] > 0 ? round(($item['present_count'] / $item['total']) * 100, 1) : 0 }}%)
                        </span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800">
                            {{ $item['absent_count'] }} ({{ $item['total'] > 0 ? round(($item['absent_count'] / $item['total']) * 100, 1) : 0 }}%)
                        </span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-yellow-100 text-yellow-800">
                            {{ $item['late_count'] }} ({{ $item['total'] > 0 ? round(($item['late_count'] / $item['total']) * 100, 1) : 0 }}%)
                        </span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-blue-100 text-blue-800">
                            {{ $item['excused_count'] }} ({{ $item['total'] > 0 ? round(($item['excused_count'] / $item['total']) * 100, 1) : 0 }}%)
                        </span>
                    </td>
                </tr>
                @endforeach
