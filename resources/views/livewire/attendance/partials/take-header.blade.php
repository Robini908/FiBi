    ...
    <div class="flex flex-wrap items-center justify-between">
        <div>
            <h1 class="text-xl font-semibold text-gray-900">
                @if($classId && $sectionId && isset($class) && isset($section))
                    Take Attendance - {{ $class->name ?? 'Class' }} {{ $section->name ?? 'Section' }}
                @else
                    Take Attendance
                @endif
            </h1>
        ...
        @if($classId && $sectionId)
            <div class="flex space-x-3 items-center mt-3 sm:mt-0">
                ...
                <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium {{ $existingRecord ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
                    @if($existingRecord)
