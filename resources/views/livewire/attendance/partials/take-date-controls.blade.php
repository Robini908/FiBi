    ...
    <div class="px-6 py-4">
        <div class="grid grid-cols-1 gap-y-4 sm:grid-cols-3 sm:gap-x-4">
            <div>
                ...
                <p class="mt-1 text-xs text-gray-500">Today: {{ now()->format('M d, Y') }}</p>
            </div>
        ...
        <div class="mt-4 pt-3 border-t border-gray-100">
            <div class="flex flex-wrap gap-2">
                ...
                <button type="button" wire:click="$set('attendanceDate', '{{ now()->format('Y-m-d') }}')" 
                ...
                <button type="button" wire:click="$set('attendanceDate', '{{ now()->subDay()->format('Y-m-d') }}')" 
                    class="inline-flex items-center px-2.5 py-1 border border-gray-300 shadow-sm text-xs font-medium rounded text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-1 focus:ring-green-500">
                    Yesterday
                </button>
                <button type="button" wire:click="$set('attendanceDate', '{{ now()->addDay()->format('Y-m-d') }}')" 
                    class="inline-flex items-center px-2.5 py-1 border border-gray-300 shadow-sm text-xs font-medium rounded text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-1 focus:ring-green-500">
                    Tomorrow
                </button>
            </div>
