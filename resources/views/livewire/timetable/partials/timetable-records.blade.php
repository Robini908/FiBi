<div class="bg-white rounded-xl shadow-sm overflow-hidden">
    <div class="px-5 py-3 flex flex-col sm:flex-row sm:justify-between sm:items-center space-y-2 sm:space-y-0 border-b border-gray-100">
        <div class="flex items-center">
            ...
            <div>
                ...
                <p class="text-xs text-gray-500">
                    @if($timetableRecords->isNotEmpty())
                    {{ $timetableRecords->count() }} records found
                    @else
        ...
        <button 
            ...
            class="{{ $classId ? 'bg-green-600 hover:bg-green-700 text-white' : 'bg-gray-200 text-gray-500 cursor-not-allowed' }} inline-flex items-center px-3 py-2 rounded-lg text-xs font-medium focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition-all duration-200 shadow-sm"
            {{ $classId ? '' : 'disabled' }}>
    ...
    @if($showModal)
        <div class="m-4 bg-gray-50 p-4 rounded-lg shadow-sm border border-gray-100">
            <div class="flex items-center mb-3">
                ...
                <h3 class="text-base font-medium text-gray-800">{{ $timetableRecordForm['id'] ? 'Edit' : 'Create' }} Timetable Record</h3>
            </div>
            ...
            <form wire:submit.prevent="saveTimetableRecord">
                <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    <div>
                        <div class="flex justify-between items-center mb-1">
                            ...
                            @if(isset($timetableRecordForm['is_auto_generated']) && $timetableRecordForm['is_auto_generated'])
                        ...
                        <div class="relative rounded-md shadow-sm">
                            ...
                            @error('timetableRecordForm.name') <p class="mt-1 text-xs text-red-600">{{ $message ?? 'Invalid timetable name' }}</p> @enderror
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        @error('timetableRecordForm.description') <p class="mt-1 text-xs text-red-600">{{ $message ?? 'Invalid description' }}</p> @enderror
                    </div>
                ...
                <div class="mt-4 flex justify-end space-x-3">
                    ...
                    <button type="submit" 
                        ...
                        {{ $timetableRecordForm['id'] ? 'Update' : 'Create' }}
                    </button>
                </div>
            </form>
        </div>
        ...
        @if($timetableRecords->isEmpty())
            <div class="text-center py-8">
                ...
                <div class="mt-3">
                    <button 
                        wire:click="createTimetableRecord"
                        class="{{ $classId ? 'bg-green-600 hover:bg-green-700 text-white' : 'bg-gray-200 text-gray-500 cursor-not-allowed' }} inline-flex items-center px-3 py-2 text-xs font-medium rounded-lg focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition-all duration-200 shadow-sm"
                        {{ $classId ? '' : 'disabled' }}>
                        <svg class="w-3.5 h-3.5 mr-1.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                        </svg>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach($timetableRecords as $record)
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                ...
                                <td class="px-3 py-2 whitespace-nowrap text-xs text-gray-500">{{ $record->description ?? '-' }}</td>
                                ...
                                <td class="px-3 py-2 whitespace-nowrap text-xs text-right font-medium space-x-1">
                                    <button wire:click="editTimetableRecord({{ $record->id }})" class="text-green-600 hover:text-green-900 focus:outline-none">
                                        Edit
                                    </button>
                                    <span class="text-gray-300">|</span>
                                    <button wire:click="confirmDeleteTimetableRecord({{ $record->id }})" class="text-red-600 hover:text-red-900 focus:outline-none">
                                        Delete
                                    </button>
                                </td>
                            </tr>
