<div class="p-6">
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div class="md:col-span-1 bg-gray-50 p-4 rounded-lg border border-gray-200">
            ...
            <div class="mb-4">
                ...
                <select id="filterAcademicSession" wire:model.live="filterAcademicSession" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-green-500 focus:border-green-500 sm:text-sm rounded-md">
                    ...
                    @foreach($availableAcademicSessions as $session)
            ...
            <div class="mb-4">
                ...
                <select id="filterAcademicTerm" wire:model.live="filterAcademicTerm" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-green-500 focus:border-green-500 sm:text-sm rounded-md">
                    ...
                    @foreach($availableAcademicTerms as $term)
        ...
        <div class="md:col-span-2 bg-gray-50 p-4 rounded-lg border border-gray-200">
            ...
            @else
                            ...
                            <li class="p-3 hover:bg-gray-50">
                                <label class="flex items-start">
                                    ...
                                    <div class="ml-3 text-sm">
                                        <p class="font-medium text-gray-700">{{ $timetable->name }}</p>
                                        <div class="text-gray-500 flex flex-wrap gap-2 mt-1">
                                            <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-100 text-gray-800">
                                                Class: {{ optional($timetable->myClass)->name ?? 'Unknown' }}
                                            </span>
                                            <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-100 text-gray-800">
                                                Session: {{ $timetable->academic_session ?? 'Unknown' }}
                                            </span>
                                            <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-green-100 text-green-800">
                                                Term: {{ $timetable->academic_term ?? 'Unknown' }}
                                            </span>
                                        </div>
                                    </div>
                                </label>
                ...
                <div class="mt-2 text-sm text-gray-500">
                    Selected: {{ count($selectedTimetableIds) }} of {{ $availableTimetables->count() }} timetables
                </div>
                
                @error('selectedTimetableIds')
        ...
        <div class="md:col-span-1 bg-gray-50 p-4 rounded-lg border border-gray-200">
            ...
            <div class="mb-3 pl-7">
                <div class="flex items-start">
                    <div class="flex items-center h-5">
                        <input id="highlightConflicts" wire:model.live="consolidationOptions.highlight_conflicts" type="checkbox" class="focus:ring-green-500 h-4 w-4 text-green-600 border-gray-300 rounded" {{ !$consolidationOptions['show_conflicts'] ? 'disabled' : '' }}>
                    </div>
                    <div class="ml-3 text-sm">
                        <label for="highlightConflicts" class="font-medium text-gray-700 {{ !$consolidationOptions['show_conflicts'] ? 'text-gray-400' : '' }}">Highlight Conflicts</label>
                        <p class="text-gray-500 {{ !$consolidationOptions['show_conflicts'] ? 'text-gray-400' : '' }}">Make conflicts stand out visually</p>
                    </div>
                </div>
            </div>
    ...
    <div class="mt-6 flex justify-center">
        <button type="button" 
            ...
            {{ count($selectedTimetableIds) < 1 ? 'disabled' : '' }}>
