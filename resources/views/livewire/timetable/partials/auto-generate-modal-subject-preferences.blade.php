...
<div class="space-y-5 py-3">
    ...
    <div class="bg-green-50 p-4 rounded-lg border border-green-200 flex items-center justify-between">
        <div class="flex items-center">
            ...
            <div>
                <p class="font-medium text-green-700"><span class="text-green-600 font-bold">{{ count($autoGenerateForm['subject_preferences'] ?? []) }}</span> subjects available for timetable generation</p>
    ...
    <div class="bg-white overflow-hidden shadow-sm rounded-lg border border-gray-200">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors duration-150">
                            <td class="px-3 py-2 text-sm">
                                <div class="flex flex-col space-y-1">
                                    <div class="flex items-center">
                                        <span class="font-medium text-gray-900 truncate max-w-[120px]" title="{{ $preferences['name'] ?? 'Unknown Subject' }}">
                                            {{ $preferences['name'] ?? 'Unknown Subject' }}
                                        </span>
                                        <span class="ml-1.5 inline-flex items-center px-1.5 py-0.5 rounded-full text-xs font-medium {{ $categoryBadgeClass }}">
                                            {{ $category }}
                                        </span>
                                    </div>
                                    @if(isset($preferences['teacher_name']) && $preferences['teacher_name'])
                                        <span class="text-xs text-green-600 flex items-center">
                                            ...
                                            <span class="truncate max-w-[120px]" title="{{ $preferences['teacher_name'] }}">{{ $preferences['teacher_name'] }}</span>
                                        </span>
                                    @endif
                                </div>
                            </td>
                            <td class="px-3 py-2 text-sm">
                                <div x-data="{ time: '{{ $preferredTime }}' }">
                                    <div class="flex flex-wrap gap-1.5">
                                        <label @click="time = 'morning'" 
                                            ...
                                            <input type="radio" 
                                                   x-model="time"
                                                   name="time_pref_{{ $subjectId }}" 
                                                   value="morning" 
                                                   wire:model.defer="autoGenerateForm.subject_preferences.{{ $subjectId }}.preferred_time" 
                                                   class="sr-only">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            <td class="px-3 py-2 text-sm text-center">
                                <div class="flex justify-center">
                                    <input 
                                        ...
                                        min="1" 
                                        max="5" 
                                        value="{{ $preferences['max_consecutive'] ?? 2 }}"
                                        wire:model.defer="autoGenerateForm.subject_preferences.{{ $subjectId }}.max_consecutive" 
                                ...
                                <div class="flex flex-col">
                                    <div class="flex justify-center mb-1">
                                        <input 
                                            ...
                                            min="0" 
                                            max="10" 
                                            value="{{ $preferences['weekly_frequency'] ?? 3 }}"
                                            wire:model.defer="autoGenerateForm.subject_preferences.{{ $subjectId }}.weekly_frequency" 
                                            class="w-12 text-center shadow-sm sm:text-sm border-gray-300 rounded-md focus:ring-green-500 focus:border-green-500">
                                    </div>
                                    <div class="w-full bg-gray-200 rounded-full h-1.5">
                                        <div class="bg-green-600 h-1.5 rounded-full" style="width: {{ min(100, (($preferences['weekly_frequency'] ?? 3) / 10) * 100) }}%"></div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-3 py-2 text-sm">
                                <select
                                    wire:model.defer="autoGenerateForm.subject_preferences.{{ $subjectId }}.daily_limit" 
        ...
        <div class="p-3 border-t border-gray-200 bg-gray-50 grid grid-cols-2 md:grid-cols-4 gap-2 text-xs text-gray-500">
            ...
            @foreach($displayCategories as $category => $colorClass)
                <div class="flex items-center">
                    <div class="flex-shrink-0 w-4 h-4 mr-2 rounded-full {{ $colorClass }}"></div>
