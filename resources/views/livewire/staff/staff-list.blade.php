<div>
    ...
    @if(session()->has('message'))
        <x-toast-notification type="{{ session('type') }}" message="{{ session('message') }}" show="true" />
    @endif
    ...
    <div class="bg-white rounded-lg shadow-sm overflow-hidden">
        ...
        <!-- Filters Section -->
        @if($showFilters)
            @include('livewire.staff.partials.filters')
        @endif
        
        <!-- Staff List -->
        @if($staff->isEmpty())
            @include('livewire.staff.partials.empty-state', ['message' => 'No staff members found matching your criteria.'])
        @else
                    ...
                    <thead class="bg-gray-50">
                        <tr>
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                <button wire:click="sortBy('name')" class="group inline-flex items-center space-x-1">
                                    <span>Name</span>
                                    @if($sortField === 'name')
                                        ...
                                        @endif
                                    @else
                                        <svg class="h-4 w-4 text-gray-200 group-hover:text-gray-300" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                                            <path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd" />
                                        </svg>
                                    @endif
                                </button>
                            ...
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                <button wire:click="sortBy('email')" class="group inline-flex items-center space-x-1">
                                    <span>Email</span>
                                    @if($sortField === 'email')
                                        @if($sortDirection === 'asc')
                                            <svg class="h-4 w-4 text-gray-400 group-hover:text-gray-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex justify-end space-x-2">
                                        <button wire:click="viewStaff({{ $member->id }})" class="text-gray-600 hover:text-gray-900">
                                            <svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                            </svg>
                                        </button>
                                        <button wire:click="editStaff({{ $member->id }})" class="text-gray-600 hover:text-gray-900">
                                            <svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                            </svg>
                                        </button>
                                        <button wire:click="confirmDelete({{ $member->id }})" class="text-red-600 hover:text-red-900">
                                            <svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <!-- Modals -->
    @include('livewire.staff.partials.create-modal')
    @include('livewire.staff.partials.edit-modal')
    @include('livewire.staff.partials.view-modal')
    @include('livewire.staff.partials.delete-modal')
</div> 