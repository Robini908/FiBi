<form wire:submit.prevent="saveGradingSystem" x-data="{ 
    currentFocus: 0,
    rangeCount: @entangle('gradeRanges').length,
    init() {
    ...
    <div class="space-y-6">
        ...
        <div class="bg-white p-5 rounded-lg shadow-sm border border-gray-200">
                ...
                <div class="overflow-hidden bg-gray-50 rounded-lg border border-gray-200">
                    ...
                    <div class="divide-y divide-gray-200 max-h-80 overflow-y-auto" id="grade-ranges-container">
                        @foreach($gradeRanges as $index => $range)
                            <div class="p-3 grid grid-cols-12 gap-2 items-center hover:bg-gray-50" :class="{'bg-green-50': currentFocus === {{ $index }}}" id="grade-range-row-{{ $index }}">
                                <div class="col-span-1 text-center font-medium text-gray-700">{{ $index + 1 }}</div>
                                
                                <div class="col-span-2">
                                    <input
                                        type="text"
                                        wire:model="gradeRanges.{{ $index }}.grade"
                                        id="grade-{{ $index }}"
                                    ...
                                    >
                                    @error("gradeRanges.{$index}.grade") <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                                </div>
                                ...
                                <div class="col-span-3">
                                    <div class="flex items-center">
                                        <input
                                            type="number"
                                            wire:model="gradeRanges.{{ $index }}.min_score"
                                            id="min-score-{{ $index }}"
                                            min="0"
                                            max="100"
                                            step="0.01"
                                    ...
                                    @error("gradeRanges.{$index}.min_score") <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                                </div>
                                ...
                                <div class="col-span-3">
                                    <div class="flex items-center">
                                        <input
                                            type="number"
                                            wire:model="gradeRanges.{{ $index }}.max_score"
                                            id="max-score-{{ $index }}"
                                            min="0"
                                            max="100"
                                            step="0.01"
                                    ...
                                    @error("gradeRanges.{$index}.max_score") <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                                </div>
                                
                                <div class="col-span-2">
                                    <input
                                        type="text"
                                        wire:model="gradeRanges.{{ $index }}.remark"
                                        id="remark-{{ $index }}"
                                        class="focus:ring-green-500 focus:border-green-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"
                                    ...
                                    >
                                    @error("gradeRanges.{$index}.remark") <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                                </div>
                ...
                <div class="mt-4 px-2">
                    ...
                    <div class="h-8 w-full bg-gray-200 rounded-md relative overflow-hidden">
                        ...
                        <div class="absolute top-0 w-full h-full">
                            @for($i = 0; $i <= 100; $i += 10)
                                <div class="absolute h-2 border-l border-gray-400" style="left: {{ $i }}%"></div>
                                <div class="absolute text-xs text-gray-600" style="left: {{ $i }}%; top: 10px">{{ $i }}</div>
                            @endfor
                        </div>
                    </div>
                </div>
            ...
            @endif
            @error('gradeRanges') <div class="mt-1 text-red-500 text-sm">{{ $message }}</div> @enderror
        </div>
        ...
        <div class="pt-5 border-t border-gray-200 flex justify-end space-x-3">
            ...
            >
                {{ $isEditingGradingSystem ? 'Update Grading System' : 'Create Grading System' }}
            </button>
        </div>
    </div>
</form> 