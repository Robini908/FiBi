    ...
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-3xl sm:w-full">
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
                                <div>
                                    ...
                                    <span class="block mt-1 text-sm text-gray-900">{{ $selectedExam->name }}</span>
                                </div>
                                <div>
                                    <span class="block text-sm font-medium text-gray-500">Term</span>
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 mt-1">
                                        {{ isset($terms[$selectedExam->term]) ? $terms[$selectedExam->term] : 'Term ' . $selectedExam->term }}
                                    </span>
                                </div>
                                <div>
                                ...
                                <!-- Exam Schedule -->
                                @if($selectedExam->exam_date || $selectedExam->start_time || $selectedExam->end_time)
                                <div class="col-span-2 mt-2">
                                    ...
                                    <div class="flex flex-wrap gap-4 mt-1">
                                        ...
                                        <span class="inline-flex items-center text-sm text-gray-900">
                                            ...
                                            {{ \Carbon\Carbon::parse($selectedExam->exam_date)->format('d M, Y') }}
                                        </span>
                                        @endif

                                        @if($selectedExam->start_time && $selectedExam->end_time)
                                        <span class="inline-flex items-center text-sm text-gray-900">
                                            <svg class="mr-1.5 h-4 w-4 text-gray-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                                                <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clip-rule="evenodd" />
                                            </svg>
                                            {{ \Carbon\Carbon::parse($selectedExam->start_time)->format('h:i A') }} -
                                            {{ \Carbon\Carbon::parse($selectedExam->end_time)->format('h:i A') }}
                                        </span>
                                        @endif
                                    </div>
                                </div>
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-2 gap-4 text-sm">
                                <div>
                                    ...
                                    <span class="block mt-1 text-sm text-gray-900">{{ $selectedExam->created_at->format('d M, Y H:i A') }}</span>
                                </div>
                                <div>
                                    <span class="block text-sm font-medium text-gray-500">Last Updated</span>
                                    <span class="block mt-1 text-sm text-gray-900">{{ $selectedExam->updated_at->format('d M, Y H:i A') }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse border-t border-gray-200">
                @if($selectedExam && Qs::isAdministratorOrTeacher())
                <button wire:click="edit({{ $selectedExam->id ?? 0 }})" type="button" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-blue-600 text-base font-medium text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 sm:ml-3 sm:w-auto sm:text-sm transition-colors duration-200">
                    Edit Exam
                </button>
                @endif
