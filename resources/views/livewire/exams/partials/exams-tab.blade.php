    ...
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @if(isset($exams) && method_exists($exams, 'total') && $exams->total() > 0)
                        ...
                        <tr class="hover:bg-gray-50 transition-colors duration-150">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                                    {{ $terms[$exam->term] ?? "Term {$exam->term}" }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($exam->gradingSystem)
                                    <button wire:click="showGradingSystemDetails({{ $exam->grading_system_id }})" class="inline-flex items-center text-sm text-green-600 hover:text-green-900 focus:outline-none transition-colors duration-200 px-2 py-1 rounded-md hover:bg-green-50">
                            ...
                            <td class="px-6 py-4">
                                @if($exam->examSchedules && $exam->examSchedules->count() > 0)
                                    @php
                                    ...
                                    <div class="space-y-2">
                                        @foreach($groupedSchedules as $classId => $schedules)
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end space-x-3">
                                    <button wire:click="showDetails({{ $exam->id }})" class="p-1.5 bg-green-50 text-green-600 hover:text-green-900 hover:bg-green-100 rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition-colors duration-200" data-tippy-content="View Details">
                                    ...
                                    @if(Qs::isAdministratorOrTeacher())
                                        <button wire:click="edit({{ $exam->id }})" class="p-1.5 bg-green-50 text-green-600 hover:text-green-900 hover:bg-green-100 rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition-colors duration-200" data-tippy-content="Edit Exam">
                                            <svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                                                <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" />
                                            </svg>
                                        </button>

                                        <button wire:click="confirmDelete({{ $exam->id }})" class="p-1.5 bg-red-50 text-red-600 hover:text-red-900 hover:bg-red-100 rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 transition-colors duration-200" data-tippy-content="Delete Exam">
                                            <svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                        ...
                        <td colspan="6" class="px-6 py-10 text-center">
                            <div class="flex flex-col items-center justify-center py-8">
                                ...
                                <p class="mt-1 text-sm text-gray-500 max-w-md">
                                    @if($search || $filterYear || $filterTerm || $filterGrading)
                                        No exams match your filter criteria. Try adjusting your filters.
                                    @else
                                        Get started by creating a new exam.
                                    @endif
                                </p>
                                @if(Qs::isAdministratorOrTeacher() && !$search && !$filterYear && !$filterTerm && !$filterGrading)
                                    <div class="mt-6">
