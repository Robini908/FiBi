<div>
    <form wire:submit.prevent="save" class="space-y-6">
        ...
        <!-- Grade Ranges -->
        <div class="bg-white rounded-md shadow-sm p-4" x-data="{ animating: false }">
            <div class="flex justify-between items-center mb-4">
                ...
                <button 
                    ...
                    wire:loading.attr="disabled"
                    :disabled="$wire.saving || animating"
                    @click="animating = true; setTimeout(() => animating = false, 500)"
                >
                    <svg class="h-4 w-4 mr-1" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                    </svg>
                    <span wire:loading.remove wire:target="addGradeRange">Add Grade</span>
                    <span wire:loading wire:target="addGradeRange">Adding...</span>
                </button>
            </div>
                        ...
                        <tr x-transition>
                            ...
                            <td class="px-4 py-2 whitespace-nowrap text-right">
                                <button 
                                    ...
                                    class="text-red-600 hover:text-red-900 transition-colors duration-200 {{ count($gradeRanges) <= 1 ? 'opacity-50 cursor-not-allowed' : '' }}"
                                    {{ count($gradeRanges) <= 1 ? 'disabled' : '' }}
                                    wire:loading.attr="disabled"
                                    :disabled="$wire.saving || animating"
                                    @click="animating = true; setTimeout(() => animating = false, 500)"
                                >
                                    <span wire:loading.remove wire:target="removeGradeRange({{ $index }})">
                                        <svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                        </svg>
                                    </span>
                                    <span wire:loading wire:target="removeGradeRange({{ $index }})">
