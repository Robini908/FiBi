    ...
    <div x-data="{ focusedField: null }" class="space-y-6 bg-white rounded-lg shadow overflow-hidden">
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center">
                        ...
                        <div class="sm:w-80">
                            @switch($setting->type)
                                @case('textarea')
                                    <div 
                                        @focusin="focusedField = '{{ $setting->key }}'" 
                                        @focusout="focusedField = null"
                                        :class="{'ring-2 ring-blue-500 ring-opacity-50': focusedField === '{{ $setting->key }}'}"
                                        class="relative transition-all duration-150 rounded-md shadow-sm"
                                    >
                                        <textarea 
                                            id="{{ $setting->key }}" 
                                            wire:model="formValues.{{ $setting->key }}" 
                                            rows="3" 
                                ...
                                @case('boolean')
                                    <div class="flex items-center">
                                        <button 
                                            type="button" 
                                            wire:click="$set('formValues.{{ $setting->key }}', '{{ $formValues[$setting->key] == '1' ? '0' : '1' }}')"
                                            class="relative inline-flex flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 {{ $formValues[$setting->key] == '1' ? 'bg-blue-600' : 'bg-gray-200' }}"
                                            role="switch" 
                                            aria-checked="{{ $formValues[$setting->key] == '1' ? 'true' : 'false' }}"
                                        >
                                            <span 
                                                aria-hidden="true" 
                                                class="pointer-events-none inline-block h-5 w-5 rounded-full bg-white shadow transform ring-0 transition ease-in-out duration-200 {{ $formValues[$setting->key] == '1' ? 'translate-x-5' : 'translate-x-0' }}"
                                            ></span>
                                        </button>
                                        <span class="ml-2 text-sm text-gray-700">
                                            {{ $formValues[$setting->key] == '1' ? 'Enabled' : 'Disabled' }}
                                        </span>
                                    </div>
                                    @break
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            @for($year = $startYear; $year <= $endYear; $year++)
                                                <option value="{{ $year }} to {{ $year + 1 }}">{{ $year }} - {{ $year + 1 }}</option>
                                            @endfor
                                        </select>
                                    </div>
                                    @break
        ...
        @else
            <div class="p-6 flex items-center justify-center h-32">
                <p class="text-sm text-gray-500">{{ __('No settings found for this group.') }}</p>
            </div>
        @endif
    </div>
