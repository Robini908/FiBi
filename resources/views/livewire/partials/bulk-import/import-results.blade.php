        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                    ...
                    <div class="p-4 bg-gray-50 rounded-lg border border-gray-200">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4">
                                ...
                                <p class="mt-1 text-xl font-semibold text-gray-900">{{ count($importResults['success'] ?? []) }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="p-4 bg-gray-50 rounded-lg border border-gray-200">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4">
                                ...
                                <p class="mt-1 text-xl font-semibold text-gray-900">{{ count($importResults['errors'] ?? []) }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="p-4 bg-gray-50 rounded-lg border border-gray-200">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4">
                                ...
                                <p class="mt-1 text-xl font-semibold text-gray-900">{{ count($importResults['success'] ?? []) + count($importResults['errors'] ?? []) }}</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="px-6 pt-4" x-data="{ activeTab: 'success' }">
                <div class="border-b border-gray-200">
                    <nav class="-mb-px flex space-x-8">
                        <button 
                            ...
                            Successful Imports 
                            <span class="ml-1 px-2 py-0.5 text-xs rounded-full bg-green-100 text-green-800">{{ count($importResults['success'] ?? []) }}</span>
                        </button>
                        ...
                        <button 
                            ...
                            class="whitespace-nowrap py-2 px-1 border-b-2 font-medium text-sm focus:outline-none transition-colors">
                            Failed Imports 
                            <span class="ml-1 px-2 py-0.5 text-xs rounded-full bg-red-100 text-red-800">{{ count($importResults['errors'] ?? []) }}</span>
                        </button>
                    </nav>
                </div>
                ...
                <div x-show="activeTab === 'success'" class="py-4">
                    @if(count($importResults['success'] ?? []) > 0)
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @foreach($importResults['success'] as $rowNum => $student)
                                        <tr>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $student['admission_number'] ?? 'N/A' }}</td>
                ...
                <div x-show="activeTab === 'errors'" class="py-4">
                    @if(count($importResults['errors'] ?? []) > 0)
                        <ul class="divide-y divide-gray-200">
                            @foreach($importResults['errors'] as $rowNum => $error)
                                <li class="py-4">
                                            ...
                                            <div class="bg-gray-50 p-3 rounded-md">
                                                <ul class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                                    @foreach($error['row_data'] as $field => $value)
                                                        <li class="text-sm">
                                                            <span class="font-medium text-gray-500">{{ ucfirst(str_replace('_', ' ', $field)) }}:</span>
            ...
            <div class="px-6 py-4 bg-gray-50 border-t border-gray-200">
                ...
                <button 
                    ...
                    @if(empty($importResults['errors'])) disabled @endif>
