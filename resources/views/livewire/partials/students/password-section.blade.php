...
<div class="mb-6 bg-white rounded-lg shadow-sm overflow-hidden">
    <div class="flex justify-between items-center p-4 bg-gray-50 border-b border-gray-100">
        ...
        <button 
            wire:click="$set('editPassword', {{ !$editPassword }})" 
        ...
        >
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $editPassword ? 'M6 18L18 6M6 6l12 12' : 'M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z' }}" />
            </svg>
            {{ $editPassword ? 'Cancel' : 'Edit' }}
        </button>
    </div>
    <div class="p-5" x-data="{ showOldPassword: false, showNewPassword: false, showConfirmPassword: false }">
        @if ($editPassword)
            <form wire:submit.prevent="savePassword">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    @if ($userType === 'student')
                        <div>
                            ...
                            @error('old_password') <p class="mt-1 text-sm text-red-600">{{ $message ?? 'Please enter your current password.' }}</p> @enderror
                        </div>
                    @endif
                    ...
                    <div>
                        ...
                        @error('password') <p class="mt-1 text-sm text-red-600">{{ $message ?? 'Please enter a valid password.' }}</p> @enderror
                    </div>
                    ...
                    <div>
                        ...
                        @error('password_confirmation') <p class="mt-1 text-sm text-red-600">{{ $message ?? 'Password confirmation does not match.' }}</p> @enderror
                    </div>
                ...
                <div class="mt-6 flex items-center justify-start space-x-3">
                    ...
                    <button type="button" wire:click="$set('editPassword', {{ !$editPassword }})" class="inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md shadow-sm text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
                        Cancel
                    </button>
                </div>
            </form>
        @else
            <div class="flex items-center justify-between p-4 bg-gray-50 rounded-md">
                <div class="flex items-center">
                    ...
                    <p class="text-sm text-gray-600">Password is hidden for security reasons. Click "Edit" to update the password.</p>
                </div>
                <div>
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                        <svg xmlns="http://www.w3.org/2000/svg" class="-ml-0.5 mr-1.5 h-3 w-3 text-green-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
