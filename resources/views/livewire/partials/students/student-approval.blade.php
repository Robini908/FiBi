            ...
            <div class="p-5">
                <div class="flex items-center">
                    ...
                    <div class="flex-shrink-0">
                        @if ($selectedStudent->photo)
                            <img src="{{ asset($selectedStudent->photo) }}" alt="{{ $selectedStudent->first_name }}" 
                                class="h-16 w-16 rounded-full object-cover border-2 border-green-100">
                        @else
                            <div class="h-16 w-16 rounded-full bg-green-100 flex items-center justify-center">
                                <span class="text-green-700 font-medium text-lg">{{ substr($selectedStudent->first_name, 0, 1) }}{{ substr($selectedStudent->last_name, 0, 1) }}</span>
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="ml-5 flex-1">
                        <h4 class="text-lg font-medium text-gray-900">{{ $selectedStudent->first_name }} {{ $selectedStudent->last_name }}</h4>
                        <p class="text-sm text-gray-500 mt-0.5">Admission No: {{ $selectedStudent->adm_no }}</p>
                    </div>
                ...
                <div class="mt-5 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                    <div class="px-4 py-3 bg-gray-50 rounded-lg">
                        ...
                        <p class="mt-1 text-sm font-medium text-gray-800">{{ $selectedStudent->my_class->name ?? 'N/A' }} - {{ $selectedStudent->section->name ?? 'N/A' }}</p>
                    </div>
                    
                    <div class="px-4 py-3 bg-gray-50 rounded-lg">
                        <p class="text-xs font-medium text-gray-500 uppercase tracking-wide">Year Admitted</p>
                        <p class="mt-1 text-sm font-medium text-gray-800">{{ $selectedStudent->year_admitted }}</p>
                    </div>
                    
                    <div class="px-4 py-3 bg-gray-50 rounded-lg">
                        <p class="text-xs font-medium text-gray-500 uppercase tracking-wide">Email</p>
                        <p class="mt-1 text-sm font-medium text-gray-800 truncate">{{ $selectedStudent->email ?? 'N/A' }}</p>
                    </div>
                    ...
                    <div class="px-4 py-3 bg-gray-50 rounded-lg sm:col-span-2 md:col-span-3">
                        ...
                        <div class="mt-1 flex items-center">
                            ...
                            <p class="text-sm font-medium text-gray-800">
                                {{ $selectedStudent->parent_detail->parent_first_name ?? 'N/A' }} {{ $selectedStudent->parent_detail->parent_last_name ?? 'N/A' }}
                                @if ($selectedStudent->parent_detail && $selectedStudent->parent_detail->parent_phone_number)
                                <span class="text-gray-500 ml-3">
                                    ...
                                    {{ $selectedStudent->parent_detail->parent_phone_number }}
                                </span>
                                @endif
                            </p>
                        </div>
