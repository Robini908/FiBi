<div
    x-data="{ modalOpen: @entangle('showViewModal').live }"
            ...
            <div class="px-6 py-4 bg-white sm:p-6 max-h-[calc(90vh-8rem)] overflow-y-auto">
                ...
                <div class="flex flex-col md:flex-row md:justify-between md:items-center border-b pb-4 mb-4">
                    <div>
                        ...
                        <span class="ml-2 font-bold text-green-700">{{ $selectedVoucher->voucher_number }}</span>
                    </div>
                    <div class="flex mt-2 md:mt-0">
                        <span class="text-sm text-gray-500 mr-2">Status:</span>
                        @if($selectedVoucher->is_cancelled)
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800">
                                Cancelled
                            </span>
                            @if($selectedVoucher->cancellation_reason)
                                <span class="ml-2 text-xs text-gray-500">({{ $selectedVoucher->cancellation_reason }})</span>
                            @endif
                        ...
                        @elseif($selectedVoucher->status === 'paid')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <p class="mt-1">{{ $selectedVoucher->votehead->name }}</p>
                            <p class="text-xs text-gray-500">{{ $selectedVoucher->votehead->code }}</p>
                        </div>

                        <div>
                            <h3 class="text-sm font-medium text-gray-500">Amount</h3>
                            <p class="mt-1 font-bold text-green-700">KES {{ number_format($selectedVoucher->amount, 2) }}</p>
                        </div>

                        <div>
                            <h3 class="text-sm font-medium text-gray-500">Payment Method</h3>
                            <p class="mt-1">{{ ucfirst($selectedVoucher->payment_method) }}</p>
                            @if($selectedVoucher->payment_method === 'cheque' && $selectedVoucher->cheque_number)
                                <p class="text-xs text-gray-500">Cheque #: {{ $selectedVoucher->cheque_number }}</p>
                            @endif
                        </div>

                        <div>
                            <h3 class="text-sm font-medium text-gray-500">Purpose</h3>
                            <p class="mt-1">{{ $selectedVoucher->purpose }}</p>
                        </div>

                        <div>
                            <h3 class="text-sm font-medium text-gray-500">Academic Period</h3>
                            <p class="mt-1">Year {{ $selectedVoucher->academic_year }}, Term {{ $selectedVoucher->term }}</p>
                        </div>

                        <div>
                            <h3 class="text-sm font-medium text-gray-500">Payment Date</h3>
                            <p class="mt-1">{{ date('d M, Y', strtotime($selectedVoucher->payment_date)) }}</p>
                        </div>
                    </div>
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <p class="mt-1 font-medium">{{ $selectedVoucher->recipient_name }}</p>
                            @if($selectedVoucher->recipient_id_number)
                                <p class="text-xs text-gray-500">ID: {{ $selectedVoucher->recipient_id_number }}</p>
                            @endif
                            @if($selectedVoucher->recipient_phone)
                                <p class="text-xs text-gray-500">Phone: {{ $selectedVoucher->recipient_phone }}</p>
                            @endif
                            @if($selectedVoucher->recipient_address)
                                <p class="text-xs text-gray-500">Address: {{ $selectedVoucher->recipient_address }}</p>
                            @endif
                        </div>

                        <div>
                            <h3 class="text-sm font-medium text-gray-500">Description</h3>
                            <p class="mt-1">{{ $selectedVoucher->description }}</p>
                        </div>
                        ...
                        <div class="print:hidden">
                            ...
                            <p class="mt-1">
                                <a href="{{ Storage::url($selectedVoucher->attachment_path) }}" target="_blank" class="text-green-600 hover:text-green-800 underline">
                                    View Attachment
                                </a>
                            </p>
                        </div>
                        ...
                        <div>
                            ...
                            <p class="mt-1">{{ $selectedVoucher->created_by }}</p>
                            <p class="text-xs text-gray-500">{{ date('d M, Y H:i', strtotime($selectedVoucher->created_at)) }}</p>
                        </div>

                        @if($selectedVoucher->approved_by)
                        <div>
                            <h3 class="text-sm font-medium text-gray-500">Approved By</h3>
                            <p class="mt-1">{{ $selectedVoucher->approved_by }}</p>
                            <p class="text-xs text-gray-500">{{ date('d M, Y H:i', strtotime($selectedVoucher->approved_at)) }}</p>
                        </div>
                        @endif

                        @if($selectedVoucher->paid_by)
                        <div>
                            <h3 class="text-sm font-medium text-gray-500">Paid By</h3>
                            <p class="mt-1">{{ $selectedVoucher->paid_by }}</p>
                            <p class="text-xs text-gray-500">{{ date('d M, Y H:i', strtotime($selectedVoucher->paid_at)) }}</p>
                        </div>
                        @endif
                    </div>
                </div>
        ...
        <div class="print:hidden px-6 py-4 bg-gray-50 border-t border-gray-200 flex justify-end space-x-3">
            ...
            @if(!$selectedVoucher->is_cancelled)
                @if($selectedVoucher->status === 'pending')
                    ...
                    <button 
                        wire:click="$dispatch('confirmApproveVoucher', { id: {{ $selectedVoucher->id }} })"
                    ...
                    @endif
                    @if($isAdmin || $isAccountant || $selectedVoucher->created_by === auth()->user()->name)
                    <button 
                        wire:click="editVoucher({{ $selectedVoucher->id }})"
                    ...
                    @endif
                @elseif($selectedVoucher->status === 'approved' && ($isAdmin || $isAccountant))
                    <button 
                        wire:click="$dispatch('confirmPayVoucher', { id: {{ $selectedVoucher->id }} })"
                        class="btn-sm bg-green-100 border-green-200 hover:border-green-300 text-green-700 hover:text-green-800"
                    >
                        <svg class="w-4 h-4 fill-current opacity-50 shrink-0 mr-1" viewBox="0 0 16 16">
                ...
                @if($selectedVoucher->status !== 'paid' && ($isAdmin || $isAccountant))
                    <button 
                        wire:click="$dispatch('confirmCancelVoucher', { id: {{ $selectedVoucher->id }} })"
