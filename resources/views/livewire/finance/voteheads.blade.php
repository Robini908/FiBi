                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                            <div class="font-medium text-gray-900">{{ $votehead->name }}</div>
                            <div class="text-gray-500">Code: {{ $votehead->code }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {{ $votehead->account->name ?? 'N/A' }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {{ $votehead->academic_year }} (Term {{ $votehead->term }})
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                            <div>
                                <div class="text-gray-900 font-medium">Allocated: {{ number_format($votehead->allocated_amount, 2) }}</div>
                                <div class="text-gray-500">Balance: {{ number_format($votehead->balance, 2) }}</div>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            <div class="flex items-center space-x-2">
                                ...
                                <button 
                                    wire:click="toggleStatus({{ $votehead->id }})" 
                                    class="{{ $votehead->is_active ? 'text-red-600 hover:text-red-700' : 'text-green-600 hover:text-green-700' }} transition-colors duration-200"
                                    title="{{ $votehead->is_active ? 'Deactivate' : 'Activate' }}"
                                >
                                    @if($votehead->is_active)
                                ...
                                <!-- Edit Button -->
                                <button 
                                    wire:click="editVotehead({{ $votehead->id }})" 
                                ...
                                <button 
                                    ...
                                    wire:click="deleteVotehead({{ $votehead->id }})" 
            ...
            >
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <h3 class="text-lg font-medium leading-6 text-gray-900 mb-4">
                        {{ $isEditing ? 'Edit Votehead' : 'Add New Votehead' }}
                    </h3>
                    ...
                    <form wire:submit.prevent="saveVotehead">
                        <div class="grid grid-cols-1 gap-4">
                            ...
                            <div>
                                ...
                                @error('finance_account_id') <span class="text-xs text-red-500">{{ $message }}</span> @enderror
                            </div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input 
                                        ...
                                        class="mt-1 focus:ring-green-500 focus:border-green-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"
                                    >
                                    @error('academic_year') <span class="text-xs text-red-500">{{ $message }}</span> @enderror
                                </div>
