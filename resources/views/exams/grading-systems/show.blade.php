...
@section('page_title', 'Grading System Details')
...
@section('breadcrumbs')
    <nav class="text-sm" aria-label="Breadcrumb">
        <ol class="list-none p-0 inline-flex text-gray-500">
            ...
            <li class="text-gray-700">{{ $gradingSystem->name }}</li>
        </ol>
    </nav>
@endsection
    ...
    <div class="bg-white overflow-hidden shadow-sm rounded-lg">
        ...
        <div class="bg-gray-50 border-b border-gray-200 px-6 py-4">
            <div class="flex items-center">
                <h3 class="text-lg font-semibold text-gray-900">{{ $gradingSystem->name }}</h3>
                <span class="badge badge-outline ml-3">
                    {{ date('d M, Y', strtotime($gradingSystem->effective_date)) }}
                </span>
            </div>
            @if($gradingSystem->description)
                <p class="mt-2 text-sm text-gray-600">{{ $gradingSystem->description }}</p>
            @endif
        </div>
        ...
        <div class="p-6">
            ...
            <div class="mb-8">
                ...
                <div class="bg-gray-50 rounded-lg p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-900">{{ $gradingSystem->name }}</p>
                    </div>
                    
                    <div>
                        <p class="text-sm font-medium text-gray-500">Effective Date</p>
                        <p class="mt-1 text-sm text-gray-900">{{ date('d M, Y', strtotime($gradingSystem->effective_date)) }}</p>
                    </div>
                    
                    <div class="md:col-span-2">
                        <p class="text-sm font-medium text-gray-500">Description</p>
                        <p class="mt-1 text-sm text-gray-900">{{ $gradingSystem->description ?: 'No description available' }}</p>
                    </div>
                </div>
            </div>
            ...
            <div>
                <div class="flex justify-between items-center mb-3">
                    ...
                    <a 
                        href="{{ route('exams.grading-systems.ranges.edit', $gradingSystem->id) }}" 
                ...
                @if($gradingSystem->gradingRanges->count() > 0)
                        ...
                        <table class="table table-zebra w-full">
                            ...
                            <tbody class="divide-y divide-gray-200">
                                @foreach($gradingSystem->gradingRanges as $range)
                                    <tr>
                                        <td class="py-3 px-4 text-sm font-medium text-gray-900">{{ $range->grade }}</td>
                                        <td class="py-3 px-4 text-sm text-gray-700">{{ $range->min_score }}</td>
                                        <td class="py-3 px-4 text-sm text-gray-700">{{ $range->max_score }}</td>
                                        <td class="py-3 px-4 text-sm text-gray-700">{{ $range->points }}</td>
                                        <td class="py-3 px-4 text-sm text-gray-700">{{ $range->comment ?: '-' }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                ...
                @else
                    <div class="rounded-lg border-2 border-dashed border-gray-300 p-6 text-center">
                        ...
                        <div class="mt-4">
                            <a 
                                href="{{ route('exams.grading-systems.ranges.create', $gradingSystem->id) }}" 
        ...
        <div class="bg-gray-50 px-6 py-4 border-t border-gray-200 flex justify-end">
            ...
            <a 
                href="{{ route('exams.grading-systems.edit', $gradingSystem->id) }}" 
