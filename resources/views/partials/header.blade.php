...
<header class="fixed right-0 top-0 md:left-64 left-0 bg-white dark:bg-gray-800 h-16 z-40 shadow-sm border-b border-gray-100 dark:border-gray-700">
    <div class="h-full px-4 sm:px-6 flex items-center justify-between">
        ...
        <div class="flex items-center space-x-2">
            ...
            <div 
                x-data="{ open: false, unreadCount: {{ Auth::user()->unreadNotifications ? Auth::user()->unreadNotifications->count() : 0 }} }" 
                class="relative"
                ...
                >
                    <div class="p-4">
                        <div class="flex items-center justify-between mb-2">
                            ...
                            <a href="{{ route('notifications.index') }}" class="text-xs font-medium text-green-600 dark:text-green-400 hover:text-green-800 dark:hover:text-green-300">View all</a>
                        </div>
            ...
            <div 
                x-data="{ 
                    open: false, 
                    unreadCount: {{ Auth::user()->unreadMessages ? Auth::user()->unreadMessages->count() : 0 }},
                    closeAndRedirect(userId) {
                        this.open = false;
                        window.location.href = '{{ route('messages.index') }}' + (userId ? '?userId=' + userId : '');
                    }
                }" 
                class="relative"
            ...
            >
                ...
                >
                    ...
                    <div @click.stop class="max-h-[500px] overflow-hidden rounded-lg">
                        @livewire('messages', ['clickHandler' => 'function(userId) { $el.closest(\'[x-data]\')._x.$data.closeAndRedirect(userId); }'])
                    </div>
                </div>
            </div>
            ...
            <div x-data="{ open: false }" class="relative ml-2">
                ...
                >
                    ...
                    <span class="hidden md:block text-sm font-medium text-gray-700 dark:text-gray-300 max-w-[100px] truncate">{{ Str::limit(Auth::user()->name, 15) }}</span>
                ...
                <div 
                    ...
                    x-cloak
                >
                    <a href="{{ route('my_account') }}" class="block px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 hover:text-green-600 dark:hover:text-green-400 transition-colors duration-200" role="menuitem">
                        <div class="flex items-center">
