<!-- Form Modal -->
<x-modal id="form-modal" :show="$formModalOpen" maxWidth="4xl" wire-close="formModalOpen">
    <div class="bg-white">
        <!-- Modal Header -->
        <div class="bg-gradient-to-r from-green-600 to-green-700 px-6 py-4 border-b border-gray-200">
            <div class="flex items-center justify-between">
                <h3 class="text-lg font-medium text-white drop-shadow-sm">
                    {{ $bookId ? 'Edit Book: ' . $title : 'Add New Book' }}
                </h3>
            </div>
        </div>
        ...
        <div class="bg-gray-50 border-b border-gray-200 shadow-sm">
            <nav class="flex px-6" aria-label="Tabs">
                <button 
                    ...
                    type="button" 
                    class="py-4 px-3 border-b-2 font-medium text-sm {{ $activeTab == 'basic' ? 'border-green-500 text-green-700' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }} whitespace-nowrap transition-colors duration-200"
                >
                ...
                <button 
                    ...
                    type="button" 
                    class="ml-8 py-4 px-3 border-b-2 font-medium text-sm {{ $activeTab == 'details' ? 'border-green-500 text-green-700' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }} whitespace-nowrap transition-colors duration-200"
                >
                    <span class="flex items-center">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <button 
                    ...
                    type="button" 
                    class="ml-8 py-4 px-3 border-b-2 font-medium text-sm {{ $activeTab == 'authors' ? 'border-green-500 text-green-700' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }} whitespace-nowrap transition-colors duration-200"
                >
                    <span class="flex items-center">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <form wire:submit.prevent="save">
            ...
            <div class="p-6 max-h-[calc(100vh-250px)] overflow-y-auto bg-white">
                ...
                <div class="bg-gray-50 p-4 rounded-lg shadow-inner">
                    @if($activeTab == 'basic')
                        @include('partials.library.books.form-basic')
                    @elseif($activeTab == 'details')
                        @include('partials.library.books.form-details')
                    @elseif($activeTab == 'authors')
                        @include('partials.library.books.form-authors')
                    @endif
                </div>
            </div>
